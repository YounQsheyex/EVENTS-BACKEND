...
const googleCallback = async (req, res) => {
    try {
        ...
        if (emails.length === 1) {
            ...
            return res.redirect(
                `http://localhost:5173/finalize-google?token=${tempToken}&chosenEmail=${encodeURIComponent(chosenEmail)}`
            );
        }

        // Multiple emails: frontend will ask user to choose one
        return res.redirect(
            `http://localhost:5173/choose-email?token=${tempToken}&emails=${encodeURIComponent(JSON.stringify(emails))}`
        );
    ...
    try {
        ...
        if (emails.length === 1) {
            ...
            if (!chosenEmail || !emails.includes(chosenEmail)) {
                return res.status(400).json({ message: "Choose which email to use", emails, token });
            }
        ...
        if (!user) {
            ...
            await sendGoogleWelcomeEmail({ firstname: user.firstname, email: user.email });
        }
        ...
        res.status(400).json({ message: "Error finalizing Google sign-in", error: error.message });
    }
};

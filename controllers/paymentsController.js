...
        // Fetch ticket and populate the event with only the title and eventDate fields
...
        // FIX: Added checks for transactionResult and transactionResult.data to prevent 
        // "Cannot read properties of null (reading 'status')" if Paystack returns a malformed or empty response.
        if (!transactionResult || !transactionResult.data || transactionResult.data.status !== "success") {
...
        // UPDATED: Populate the event details when fetching the ticket
...
                });
            } catch (emailError) {
                res.status(200).json({success:"false", message:"Failed to send confirmation email but payment was successful"})
...
            // UPDATED: Nested population to get event details (title, eventDate)
            .populate({
                path: 'ticket',

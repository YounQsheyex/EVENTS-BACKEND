...
      success: false,
      message: "Please provide all necessary fields: name, type, quantity, maxOrder, price, and status"
    });
  }

 if (!status) updateFields.status = "available";
...
        success: false,
        message: "A ticket with this name already exists for this event. Please use a different name or update the existing ticket."
      });
    }
...
  try {
    // Correctly using the retrieved Model (Ticket)
    const allTicket = await Ticket.find({event: eventId}).populate('event', 'title eventDate') 
